...
extension NoticeDetailViewModel {
    ...
    @MainActor
    public func fetchNoticeDetail() async {
    ...
    public static func mergeFetchedNoticeDetail(
        fetched: NoticeDetail,
        fallback: NoticeDetail?
    ) -> NoticeDetail {
        guard let fallback else { return fetched }

        let resolvedGeneration = (!fetched.generation.isEmpty && fetched.generation != "0") ? fetched.generation : fallback.generation
    ...
    private static func resolvedTargetAudience(
        ...
        let resolvedGeneration = (!primary.generation.isEmpty && primary.generation != "0") ? primary.generation : fallback.generation
    ...
    /// 공지 리소스 권한 조회
    @MainActor
    public func fetchNoticePermission() async {
        #if DEBUG
    ...
    public func showDeleteConfirmation(onDeleteRequested: @escaping () -> Void) {
    ...
    /// 공지 상세 진입 시 1회 읽음 처리
    @MainActor
    public func markAsReadIfNeeded() async -> Bool {
        guard !hasMarkedAsRead else { return false }
        guard !noticeID.isEmpty && noticeID != "0" else { return false }
    ...
    /// 리마인더 발송
    @MainActor
    public func sendReminder(targetIds: [String]) async {
        do {
            #if DEBUG
    ...
    @MainActor
    public func sendReminderToAllUnreadUsers() async {
        do {
            var cursor = 0
            var hasNext = true
    ...
    /// 공지사항 수정 완료 (제목, 본문)
    @MainActor
    public func updateNoticeContent(title: String, content: String) async {
        noticeState = .loading
    ...
    /// 공지사항 링크 수정
    @MainActor
    public func updateNoticeLinks(links: [String]) async {
        do {
    ...
    /// 공지사항 이미지 수정
    @MainActor
    public func updateNoticeImages(imageIds: [String]) async {
        do {
    ...
    @MainActor
    public func handleVote(optionIds: [String]) async {
        ...
        do {
            ...
            if case .requestFailed(_, let data) = error,
               ...
               let parsed = try? JSONDecoder().decode(
                     VoteErrorResponse.self, from: data
               ) {
                alertPrompt = AlertPrompt(
                    id: .init(),
                    title: "투표 실패",
    ...
    @MainActor
    public func handleUpdateVote(optionIds: [String]) async {
    ...
    private struct VoteErrorResponse: Decodable {
        let message: String?
    }
}
