...
extension NoticeEditorViewModel {
    ...
    @MainActor
    public func requestAIImprovement() {
    ...
    @MainActor
    public func startAIImprovement() async {
    ...
    @MainActor
    public func improveContentWithAI() async {
        let plainText = richAttributedContent.string.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !plainText.isEmpty else { return }
    ...
    @MainActor
    public func dismissAICompletionSummary() {
        showAICompletionSummary = false
        aiStreamingText = ""
    }
    ...
    @MainActor
    public func restoreDailyTokenUsage() {
