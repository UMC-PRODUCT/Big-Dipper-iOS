...
extension EditorToolbarViewModel {
    ...
    public func clampedSelectedRange(in storage: NSTextStorage) -> NSRange {
    ...
    public func safeAttributeLocation(for range: NSRange, in storage: NSTextStorage) -> Int {
    ...
    public func currentParagraphRange(in storage: NSTextStorage) -> NSRange {
    ...
    public func isAtEOFEmptyParagraph(in storage: NSTextStorage) -> Bool {
        let clampedRange = clampedSelectedRange(in: storage)
    ...
    public func selectedParagraphRange(in storage: NSTextStorage) -> NSRange {
        guard storage.length > 0 else {
            return NSRange(location: 0, length: 0)
        }
    ...
    public func paragraphRanges(in range: NSRange, storage: NSTextStorage) -> [NSRange] {
    ...
    public func syncRange(in storage: NSTextStorage) -> NSRange? {
        let clampedRange = clampedSelectedRange(in: storage)
        if clampedRange.length > 0 {
            return clampedRange
        }
    ...
    public func adjustSelectedRange(forReplacing range: NSRange, with replacementLength: Int) {
